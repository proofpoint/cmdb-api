...
# database connection
#my $dbh=DBI->connect("DBI:mysql:database=inventory;host=$DBHOST",$DBUSER,$DBPASS);
our $dbh;
...
sub doGenericPUT
{
	...
	my $dbs=DBI->connect("DBI:mysql:database=inventory;host=$DBHOST",$DBUSER,$DBPASS,{AutoCommit=>1});
	$dbs->begin_work;
	my (@sql,$parms,@errors);
	my $data=&eat_json($$requestObject{'body'},{allow_nonref=>1});	
